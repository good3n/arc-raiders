...
export default function MapExplorer() {
  ...
  const endpoint = useMemo(() => {
    const params = new URLSearchParams({ mapID: selectedMap });
      ...
      <article key={item.id || index} className="rounded-2xl border border-[#442A50] shadow-sm p-4 bg-[#1B0F21]">
        <div>
          <header className="mb-3">
            ...
            {type && (
              <span className="inline-block mt-1 px-2 py-1 text-xs rounded-md bg-[#442A50] text-gray-200">
                {type}
              </span>
            )}
          </header>
          ...
          <div className="space-y-2 text-sm">
            {zone && (
              <div className="flex items-center text-gray-400">
                ...
                <span className="ml-2 text-gray-300">{zone}</span>
              </div>
            )}
    ...
    <div>
      ...
      <section className="grid grid-cols-3 gap-3">
        {selectedMap && filtered && filtered.length > 0 ? (
          ...
          <div className="text-sm text-slate-500">No locations found on {selectedMap}.</div>
        ) : null}
      </section>
    </div>
  );
